...
// Auto-defaults allocation for winner prize distribution (Solo, Duo, and Squad with 4 Teams)
window.updateWinnerPrizeDistribution = function() {
  ...
  if (mode === 'Solo') {
    ...
    if (input) {
      input.value = prize; // 100% of winner prize
    }
  } else if (mode === 'Duo') {
    ...
    if (input1) input1.value = Math.round(prize * 0.6); // 60%
    if (input2) input2.value = prize - Math.round(prize * 0.6); // 40% (guarantees exact sum)
  } else if (mode === 'Squad') {
    ...
    if (input1) input1.value = Math.round(prize * 0.4); // 40%
    if (input2) input2.value = Math.round(prize * 0.3); // 30%
    if (input3) input3.value = Math.round(prize * 0.2); // 20%
    if (input4) input4.value = prize - (Math.round(prize * 0.4) + Math.round(prize * 0.3) + Math.round(prize * 0.2)); // 10% (guarantees exact sum)
  }
  
  window.updateLiveMatchCardPreview();
};

// Bind elements to trigger default redistribution splits summing up exactly to winner prize
document.getElementById('tourn-winner-prize')?.addEventListener('input', () => {
  const modeEl = document.getElementById('tourn-mode');
  if (!modeEl) return;
  const mode = modeEl.value;
  const prize = Number(document.getElementById('tourn-winner-prize').value || 0);
  ...
  if (mode === 'Solo') {
    ...
    const input1 = document.getElementById('tourn-duo-1st');
    const input2 = document.getElementById('tourn-duo-2nd');
    if (input1) input1.value = Math.round(prize * 0.6);
    if (input2) input2.value = prize - Math.round(prize * 0.6);
  } else if (mode === 'Squad') {
    const input1 = document.getElementById('tourn-squad-1st');
    const input2 = document.getElementById('tourn-squad-2nd');
    const input3 = document.getElementById('tourn-squad-3rd');
    const input4 = document.getElementById('tourn-squad-4th');
    if (input1) input1.value = Math.round(prize * 0.4);
    if (input2) input2.value = Math.round(prize * 0.3);
    if (input3) input3.value = Math.round(prize * 0.2);
    if (input4) input4.value = prize - (Math.round(prize * 0.4) + Math.round(prize * 0.3) + Math.round(prize * 0.2));
  }
  window.updateLiveMatchCardPreview();
});

document.getElementById('tourn-mode')?.addEventListener('change', window.updateWinnerPrizeDistribution);
...
// Add or Update tournament deployment data safely
...
if (crudForm) {
  crudForm.addEventListener('submit', async (e) => {
    ...
    // splits variables mapping including squad 4th Team
    const solo1st = Number(document.getElementById('tourn-solo-1st')?.value || 0);
    const duo1st = Number(document.getElementById('tourn-duo-1st')?.value || 0);
    const duo2nd = Number(document.getElementById('tourn-duo-2nd')?.value || 0);
    const squad1st = Number(document.getElementById('tourn-squad-1st')?.value || 0);
    const squad2nd = Number(document.getElementById('tourn-squad-2nd')?.value || 0);
    const squad3rd = Number(document.getElementById('tourn-squad-3rd')?.value || 0);
    const squad4th = Number(document.getElementById('tourn-squad-4th')?.value || 0);
    ...
    // splits logical constraint checks - total sum must equal winnerPrize exactly
    if (mode === 'Solo' && solo1st !== winnerPrize) {
      window.showToast("Solo prize distribution allocation must equal Winner Prize exactly.", "error");
      return;
    }
    if (mode === 'Duo' && (duo1st + duo2nd) !== winnerPrize) {
      window.showToast("Duo prize splits sum must equal Winner Prize exactly.", "error");
      return;
    }
    if (mode === 'Squad' && (squad1st + squad2nd + squad3rd + squad4th) !== winnerPrize) {
      window.showToast("Squad prize splits sum must equal Winner Prize exactly.", "error");
      return;
    }
    ...
    const startsAt = Timestamp.fromMillis(startsAtMillis);
...
// Load values into modification form
window.editTournament = async function(id) {
  try {
    ...
    if (docSnap.exists()) {
      ...
      const modeEl = document.getElementById('tourn-mode');
      if (modeEl) modeEl.value = t.mode || "Solo";
      ...
      // splits restoration (includes squad 4th Team)
      const solo1stEl = document.getElementById('tourn-solo-1st');
      if (solo1stEl) solo1stEl.value = t.winnerDistribution?.solo1st ?? t.solo1st ?? 0;
      
      const duo1stEl = document.getElementById('tourn-duo-1st');
      if (duo1stEl) duo1stEl.value = t.winnerDistribution?.duo1st ?? t.duo1st ?? 0;
      
      const duo2ndEl = document.getElementById('tourn-duo-2nd');
      if (duo2ndEl) duo2ndEl.value = t.winnerDistribution?.duo2nd ?? t.duo2nd ?? 0;
      
      const squad1stEl = document.getElementById('tourn-squad-1st');
      if (squad1stEl) squad1stEl.value = t.winnerDistribution?.squad1st ?? t.squad1st ?? 0;
      
      const squad2ndEl = document.getElementById('tourn-squad-2nd');
      if (squad2ndEl) squad2ndEl.value = t.winnerDistribution?.squad2nd ?? t.squad2nd ?? 0;
      
      const squad3rdEl = document.getElementById('tourn-squad-3rd');
      if (squad3rdEl) squad3rdEl.value = t.winnerDistribution?.squad3rd ?? t.squad3rd ?? 0;

      const squad4thEl = document.getElementById('tourn-squad-4th');
      if (squad4thEl) squad4thEl.value = t.winnerDistribution?.squad4th ?? t.squad4th ?? 0;
...
if (winnerPrizeInput) winnerPrizeInput.addEventListener('input', window.updateLiveMatchCardPreview);
